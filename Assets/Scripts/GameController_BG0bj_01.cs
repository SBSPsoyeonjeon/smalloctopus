...
public class GameController_BG0bj_01 : MonoBehaviour {
    ...
    IEnumerator SpawnWaves()
    {
        yield return new WaitForSeconds(startWait);//잠깐 기다렸다가
            ...
            for (int i = 0; i < hazardCount; i++) //hazardCount값 만큼 출현
            {
                Vector3 spawnPosition = new Vector3(spawnValues1.x, spawnValues1.y, spawnValues1.z); //내가 지정한 위치에서
                Quaternion spawnRotation = Quaternion.identity;
                Instantiate(BG_object1, spawnPosition, spawnRotation);//첫번째
                yield return new WaitForSeconds(spawnWait2); // 잠깐 기다려

                spawnPosition = new Vector3(spawnValues2.x, spawnValues2.y, spawnValues2.z); //내가 지정한 위치에서
                spawnRotation = Quaternion.identity;
                Instantiate(BG_object2, spawnPosition, spawnRotation);

            yield return new WaitForSeconds(spawnWait3); // 잠깐 기다려

            spawnPosition = new Vector3(spawnValues3.x, spawnValues3.y, spawnValues3.z); //내가 지정한 위치에서
            spawnRotation = Quaternion.identity;
            ...
            //break;//yield return new WaitForSeconds(waveWait);//또 기다려
        }
    }
